...
**多 AI 会话可视化管理 + 监督 + 协作 + Ruflo 集成面板**：Web GUI（Electron 可包），Node + Express + WS + Vanilla JS，前端 3 栏。**v0.50** 体验优化 8 件套（跨 session 全局搜索 ⌘⇧F / 导出 markdown / 浏览器通知 / ⌘? 速查 / ⭐ 收藏 / Prompt 模板 ⌘P / Session 分叉 / ctx 警告条）+ **v0.49** Ruflo 集成（🐝 标签页直读 SQLite/JSON + spawn ruflo CLI）+ 一轮 12 个 bug 全清（B-01~B-05 / N-01~N-20 安全沙箱+RCE 防护）+ 二轮 5 个新 bug（N-21~N-45：Reverse Tabnabbing / LoopGuard 双重累计 / CCR env 透传等）。**v0.48** 1v1 Chat 房第三种模式（跟 codex/claude 持续互聊+协作做任务）✅：新增 `mode='chat'` 第三种房间（保留 debate / squad）。**架构**：`SoloChatDispatcher` 单 turn 模式——`POST /api/rooms/:id/chat` 接收用户消息 → 追加 `room.conversation` 数组 → 拍平为 system + messages 喂 RoomAdapter → 回写 AI 回复 → WS 广播 `chat_user_msg / chat_thinking / chat_ai_msg / chat_error / chat_aborted` 5 个事件。`ChatRoomStore.create` mode 三态白名单扩展（squad/chat/debate），chat 默认配 1 个成员（用户可指定 defaultPartner，默认 codex）。**前端**：sidebar 加 `+ 新 Chat 房` 按钮，promptModal 让用户填搭子（claude/codex/ollama/minimax）；room-area 检测 `mode='chat'` 进入聊天气泡渲染——左 AI / 右用户 / 头像 + 时间戳 + tokens + markdown 渲染 + thinking 占位 pulse 动画；底部输入区 textarea + `⌘+Enter` 发送 + 中断按钮；标题 chip 加 `· Chat` 三态文本（修了之前 squad/debate 二选一漏 chat 的 bug）。**实测闭环**：(1) 真创建房间 mode=chat ✅；(2) 发"用一句中文介绍自己 + 2+2 几"→ codex 17 秒内回"我是 GPT...2+2 等于 4" ✅；(3) 发"在 /tmp/chat_test_v048.txt 创建文件并 cat 验证"→ codex 真去 shell 跑命令 + 真创建 85 字节文件 + cat 输出回报，文件实测存在 ✅；(4) 多轮上下文连贯（codex 记得自己叫 GPT）✅；(5) 前端 4 条消息正确渲染聊天气泡 + 头部标签 "· Chat" 正确 ✅。**用户可以做的事**：跟 codex/claude/Ollama 任意一个开 1v1 对话 → 持续问问题 / 让它真去做任务（写文件 / 跑命令 / 查 API）/ 多轮调整，跟 Terminal 里用 codex 体验一致但有 Web UI + 历史持久化。

**v0.47 实机循环** 阶段 4 实机找问题修复轮（playwright 真用 4 视图截图回归 + 跑 3 个 squad/debate 实测）：(1) **Hook UI 缺失**——阶段 3 加了 `/api/hooks` 端点 + WS 广播，但 inspector 安全 tab 还没显示；加 `renderHookEventsSection()` 拉 `/api/hooks?sessionId=X` 渲染 hook 事件列表（按事件类型 7 色分边：PreTool 橙 / PostTool 绿 / Session 蓝 / Stop 灰 / Notification 红 / Subagent 紫 / 默认灰），CSS `.hook-events-list/.hook-event-item` 配套，实测 POST 4 个 hook → inspector 实时显示 4 条带颜色分类 ✅。(2) **Squad PM/QA prompt 死循环回归**——v0.47 prompt 改造引入 DEV_PROMPT 强制 dev 输出 \`### 实现 / 方案 + ### 完成情况自查\` 两节固定 markdown 框架，但 PM 拆任务可能给出"不包含 markdown"这类验收标准 → QA 永远按 task.desc 字面 reject → 跑 "2+3=几" 简单任务 **5 次 reject 直接 escalate**；修两处：PM_PROMPT TASK BOUNDARY 加"task.desc 不能限制 dev 输出格式，验收只能针对实现段内容"；QA_PROMPT TOOLS GUIDANCE 加"dev 框架是系统强制的不算多余 markdown，只针对实现段判 pass/reject"；squad-limits PROMPT_VERSIONS 升 `v0.47-pm-anthropic-2 / v0.47-qa-anthropic-2`。**回归实测**：同一 2+3 任务 **1 次 pass conf=0.98** ✅。(3) **死链接 `href="#"` 点击跳顶**——hook 段"如何配置 settings.json 接入 →" 改 `<button class="link-btn">` + JS modal；新端点 `GET /api/docs/:name`（白名单 `CCR_USAGE.md / HOOKS_USAGE.md`，路径穿越自动 404）；新 `.doc-modal` 弹窗渲染 markdown，点遮罩 / ✕ 关闭。(4) **debate v0.47 prompt 改造验证**——3 字早餐任务真跑 R1-R4 完整 90s，R3 三节（共识/分歧/最终）格式严格，R4 4 节（共识方案/共识点/分歧裁决/实施清单）+ 仲裁带具体理由 + 6 步实施清单带分量。**hookEvents 持久化验证**——panel 重启 6 次后 4 条 hook 仍在（saveData/loadData 字段 OK）。**实机循环结束**：4 个真问题全修，console 仅 1 个 verbose 提示（password DOM 警告，无害），无新 P0/P1 待修。

**v0.47** 社区资源融合三件套 ✅：搜社区后挑了 3 个真实可用的方案融入，每个都验证过。**(1) Anthropic 四要素 prompt 改造**——把 11 个 system prompt (PM/DEV/QA/JUDGE x 1 + Debate ROLE/R2/R3/R4 x 4 + Watcher buildJudgePrompt + FINAL_SUMMARY) 全改成 `OBJECTIVE / OUTPUT FORMAT / TOOLS GUIDANCE / TASK BOUNDARY` 四段式（参考 anthropic.com/engineering/multi-agent-research-system 生产 agent 框架）。**实测 squad debate**：QA 真去 shell 跑 `python print_42.py` 发现 PATH 缺 python 命令直接 reject，Dev 看到打回 issue 后建软链 `ln -s python3 .../python` 修环境，QA 第二轮 pass。**prompt 版本号同步升**：squad-limits.js `PROMPT_VERSIONS` 全改 `v0.47-*-anthropic` 后缀，task.attempts/reviews 字段持久化便于将来 A/B 对照。**(2) Claude Code Router 可选 adapter**——新建 `CCRSpawnAdapter` 继承 `ClaudeSpawnAdapter`，bin 改 `ccr` 命令前加 `code` 子命令；server.js 启动 `spawnSync('which', ['ccr'])` 检测 PATH，命中才注入 roomAdapterPool 为 'ccr'，未装静默跳过零副作用。**不强制依赖**：用户 `npm i -g @musistudio/claude-code-router` 后下次启动自动启用。写 `docs/CCR_USAGE.md` 说明启用方式 / 路由规则配置 / 故障排查。**(3) 12 个 Hook 事件接收端点**——`POST /api/hooks/:event` 接收 `PreToolUse / PostToolUse / Notification / UserPromptSubmit / SessionStart / SessionEnd / Stop / SubagentStart / SubagentStop / PreCompact / PostCompact / SubagentResult` 12 种，非法事件返 400；按 `session_id` 索引到 session.hookEvents 数组（限长 200/session + 全局环形 2000），WS 广播 `hook_event` 事件给前端实时更新；持久化 `data.json` 存最近 100/session；新端点 `GET /api/hooks?sessionId=&limit=` 拉历史。写 `docs/HOOKS_USAGE.md` 给用户配 `~/.claude/settings.json` 的 hook 命令模板（直接抄 curl）。**实测**：POST 2 个 hook（SessionStart + PreToolUse）→ `{"ok":true}`；POST 非法 event → 400 拒绝；GET 返回 2 条带完整 payload。**不动用户 ~/.claude/**：panel 给端点，用户自己配 hook，避免黄线越界。**冒烟全过**：阶段 1 squad 真跑（轻量 hello 42 任务 90s 完成，QA 真验证 shell 命令）；阶段 2 启动检测 ccr 未装静默；阶段 3 三种 hook POST + GET 全过。

**v0.46** 全面布局回归（用户报"左上角布局有问题"）✅：用 playwright 实拍 1440×900 viewport 4 张截图（welcome / chat / rooms / squad），找出 4 处真实布局崩坏并全部修：(1) **brand 区挤爆**——`.brand` flex 一行塞 [icon 36 + text + 4×28 button + 3 gap]，sidebar 12% (~173px) 减去 icon/gap/button 后 brand-text 只剩 ~10px，导致 "Claude Panel" 标题被纵向单字宽压扁、副标题被截、"v0.45" 漂浮错位；改 HTML 把 4 个图标按钮拆到新 `.brand-actions` 行（独立一行齐排 6px gap），CSS `.brand-text` 加 `white-space: nowrap + ellipsis`、`.brand-icon` 加 `flex-shrink: 0`。(2) **sidebar 12% 太窄**——窄屏 (~1440) 下 173px 既挤 brand 又挤 session 名；`.app grid-template-columns` 从 `12% 60% 28%` 改 `240px 1fr 340px` 固定边栏弹性中区，宽屏不浪费、窄屏不压缩。(3) **chat-header 8 组件挤一行被压成纵向**——监视 toggle + state-chip + cost-chip + ctx-meter + chain-badge + 接力/Terminal/全开 按钮文字被压成"接力"上下两字；`.chat-header` 加 `flex-wrap: wrap`、`.chat-header-actions` 加 `flex-wrap+align-items: center+gap:6px`、加 `.cxbtn-sm/btn-icon` 内 `white-space: nowrap + padding 5px 8px`；窄屏下按钮组自然换行不再压字。(4) **Squad Kanban 5 列横向溢出**——`grid-template-columns: repeat(5, minmax(160px, 1fr))` 在 room-main ~600px 下 5×160=800 必溢出，"⚠️ 升级" 列被裁；改 `repeat(auto-fit, minmax(140px, 1fr))` 自适应换行（实拍验证 3+2 排列无横向滚动），`.room-main` 加 `min-width: 0` 防 flex 子项撑爆。(5) **statusBar 硬编码 "v0.6"**——v0.30 已让 brand-subtitle 拉 `/api/version` 但 statusVersion 漏改；同一段 fetch 顺手刷 `$('#statusVersion').textContent = 'v${r.version} · ⌘K 命令面板'`，现实拍显示 "v0.45"。**端口默认改 51735**（5173 + 5 视觉记忆，私有端口段避撞 Vite/Node）；server.js + electron-main.js 两处同步改；旧 5173 上的远古 panel(PID 61950, v0.30 之前)已 kill。**playwright 实拍验证**：4 张截图全部修后 brand 区完美、Kanban 3+2 换行、chat-header 按钮整齐、状态栏右下 v0.45 动态。

**v0.45** 三轮审查发现 v0.43/v0.44 修复引入的回归 + 新发现，P0×3+P1×5+P2×4 全部修完 ✅。**P0 修复**：(1) **Claude/Codex cleanup 闭包 TDZ**——cleanup 闭包引用 `timer`/`onAbort`，但用 `const` 声明在 cleanup 之后，spawn 立即失败时 `child.on('error')` 同步派发 → cleanup 被调时变量在 TDZ 抛 ReferenceError 进程崩；改 `let timer = null; let onAbort = null;` 预声明 + cleanup 内 null 检查。(2) **ChatRoomStore.flush() `|| true` 死代码 + 未捕获崩溃丢盘**——`if (this._savePending || true)` 永真但语义错乱；改干净的"无条件 `_doSave()` + reset pending"。server.js 加 `process.on('uncaughtException'|'unhandledRejection')` 落盘 + 调 `roomStore.flush()`，避免崩溃丢 in-flight 写。(3) **`roomAdapterPool` MiniMax 漏重建**——`buildRoomAdapters()` 启动时 build 一次，`rebuildAdapter()` 只重建 watcher 池；MiniMax key 配后 Room 路径仍 "adapter not registered"。改 `rebuildAdapter()` 末尾同步刷 `roomAdapterPool`（用 try 兜 TDZ）。**P1 修复**：(4) **squad role 非法静默 fallback 误导用户**——PATCH 收 `role: hacker` 被静默改 `dev`，PM 数变 0 → start 抛"需要 1 PM + 1 Dev + 1 QA"，用户摸不着头脑；squad 房 role 非法直接返 422 + 错误消息。(5) **throttle trailing 调用拉错房间**——`_pullRoomThrottle.timer` fire 时若 `roomState.activeId` 已切走，会渲染旧房间数据到新房间 UI；改 `targetId = roomState.activeId` capture，timer 内和 fetch 后两次比对。(6) **`watcherHistory` 没持久化**——`_performJudge` mutate `session.watcherHistory` 后没调 `saveData()`，重启丢；WatcherDispatcher 加 `persistSession` 回调 + server.js 注入 `() => saveData()`。(7) **broadcastRoom 异常打断 Promise.all**——`JSON.stringify` 循环引用时抛会让整个 batch reject + `activeAborts.delete` 后续 abort 失效；broadcastRoom 整体套 try/catch。(8) **debate 房 role undefined 行为正确**——审查报告中的"`role===observer` 污染"实际是 squad 默认才 fallback `dev`，debate 路径设 `undefined` 本就是合规设计，无需改。**P2 修复**：(9) **finalDegraded 前端未读**——judge 失败时 R3 合并的 fallback finalConsensus 没标降级，UI 看起来跟正常 judge 一样；加 `final-degraded-badge` 红色 banner 横在 markdown 之前。(10) **JSON 截断破坏花括号**——审查担心 `slice(0, max)` 切掉尾 `}` 让 PM 重试；其实 `lastIndexOf('}')` + `end <= start` 防御已经覆盖，加 `end <= start` 显式校验更稳。(11) **setTimeout sendMessageToClaude 闭包持过期 session**——watcher autoExecute 1s 延迟期间 session 被删则 NPE；改 setTimeout 内重 `sessions.get(session.id)` 比对存活。(12) **Codex/Claude stdin.write 外层 try 死代码**——`stdin.write(prompt, callback)` 是 EventEmitter 异步，外层同步 try 抓不到回调里的错；删冗余 try 块。**冒烟**：squad 422 ✅ debate 200 ✅ version 0.45 ✅；启动期 TDZ 用 try 兜底 ✅。

**v0.44** 二轮审查后修剩余 P1×3 + P2×6 ✅：(1) **P1 #13 前端多行 reasoning 坍成一行**——QA 的 `reasoning/issues/suggestions` 含 `\n` 时 escapeHtml 不转 `<br>`；新增 `escapeHtmlMl()` helper，抽屉里 reasoning/issues/suggestions 三处全换。(2) **P1 #14 DOMPurify URI 协议**——加 `ALLOWED_URI_REGEXP: /^(https?:|mailto:|tel:|#|\/)/i` 防 `data:image/svg+xml,<script>` 等 SVG XSS 绕过。(3) **P1 #15 rooms.json 写盘风暴**——squad 单跑触发 ~40 次同步 `JSON.stringify(全部 rooms) + writeFileSync`；改 `ChatRoomStore.save()` 为 250ms debounce 异步，新加 `flush()` 给 gracefulShutdown 同步落盘；server.js SIGTERM/SIGINT 补 `roomStore.flush()`。(4) **P2 #16 前端 GET 请求风暴**——`pullRoomAndRender` 加 250ms throttle + pending 标记合并尾调用。(5) **P2 #17 `_parsePlanJson` DoS**——`maxRawJsonLength = 200000` 字节截断，实测 550KB→200KB→2ms 拒绝。(6) **P2 #18 R4 失败 finalConsensus null**——judge 失败时把 R3 三方终稿合并成 markdown 作为 fallback 写入 `finalConsensus`，加 `finalDegraded: true` 标记 + WS `judge_error` 带 `fallback` 字段。(7) **P2 #19 抽 squad-limits.js**——`SQUAD_LIMITS / ROOM_LIMITS / ADAPTER_TIMEOUTS / PROMPT_VERSIONS` 4 组配置，调一次一处改。(8) **P2 #20 prompt 版本化**——`task.attempts[].promptVersion` 写 squad_dev、`task.reviews[].promptVersion` 写 squad_qa，未来 A/B 调 prompt 能按 turn 回溯。(9) **P2 #22 flattenMessages**——评估后保留分离（RoomAdapter 无截断 vs WatcherAdapter 含 sessionState 头 + 30 条 + 800 字截断，强合并风险大于收益）。**冒烟**：DoS 输入 550KB→2ms 拒绝 ✅；10 连建房后 debounce 仅 1 次落盘 + 10 条完整 ✅；6 文件语法 ✅；版本号自动跳 0.44。

**v0.43** 全面代码审查后修 P0×7 + P1×3 关键 bug ✅：派 code-reviewer agent 全面扫 v0.39-v0.42 22 条发现。**P0 修复**：(1) **CodexSpawnAdapter tmpDir 泄漏**——`mkdtempSync('codex-room-')` 只 `unlinkSync(outFile)` 不删目录，throw 路径下连 outFile 都不删，改用 `rmSync(tmpDir, {recursive:true, force:true})` + 双路径 cleanup + settled 单一终态保证。(2) **MiniMax 接口错配必崩**——v0.40 把 `MiniMaxAdapter`（只继承 `WatcherAdapter` 仅有 `judge()`）塞进 `roomAdapterPool`，dispatcher 调 `.chat()` 必抛 `is not a function`；新建 `src/room/MiniMaxChatAdapter.js extends RoomAdapter` 实现 `chat()`，server.js 改 import 路径。(3) **abortSignal listener 累积**——3 个 spawn/HTTP adapter 都 `addEventListener('abort', fn, {once:true})` 但没 `removeEventListener`，squad 跨 N task 复用同一 aborter 会堆 2N 个监听器触发 `MaxListenersExceededWarning`；3 个 adapter 全改保留 handler 引用 + `finally`/`cleanup` 路径 removeEventListener。(4) **spawn stdin EPIPE 防御**——`stdin.write(prompt); stdin.end()` 同步调用，CLI 立即 exit 或大 prompt 都会 EPIPE 抛 unhandledRejection；改 `child.stdin.on('error', ...)` 静默 EPIPE + write 用 callback 版本兜底。(5) **TaskGraph stack 跨调用残留**——DFS 在多入口时 `stack` 闭包变量共享，BLACK 节点 pop 后下一组 dfs 会含上次残留 id；改 stack 作为参数传入，每个外层 dfs 各自一份；4 个回归用例（多入口 / 两独立环 / 重叠环 / 健康 DAG）全过。(6) **rooms.json 权限**——`writeFileSync` 默认 0o644，改 `{ mode: 0o600 }` + 已存在文件显式 `chmodSync` + 目录 0o700，实测 `-rw-------`。(7) **CollaborationDispatcher room 快照失效**——Step1 后 `const room = this.store.get(roomId)` 取一次，循环里读 `room.qaStrictness/cwd` 永远是首次快照，用户中途 PATCH qaStrictness 不生效；改每次 dev/qa 调用前重拉 `this.store.get(roomId)`。**P1 修复**：(8) **PATCH members 校验**——直接覆盖无白名单，前端传脏 adapterId 后 `_callAdapter` throw 整 squad 报错；加 `roomAdapterPool.has()` 过滤 + role 限白名单 + 数组长 ≤20 + 字段长度截断。(9) **inject 长度上限**——content 无上限可撑爆 token，加 `INJECT_MAX_LEN=4000`（413）+ `INJECT_MAX_COUNT=20`（429）。(10) **WatcherDispatcher 切池子残留**——`rebuildAdapter` 每次 new 新 dispatcher 丢 sessionState 连续性 + 残留旧引用并发问题；改成复用单例调 `setAdapter / setAdapterPool / setConfig`；session 指定的 provider 被踢出池时新增 `watcher_fallback` 事件广播给前端。**冒烟**：脏 members 被规范化（FAKE_PROVIDER → claude, hacker → dev）✅；超长 inject 返 413 ✅；rooms.json 权限 0o600 ✅；TaskGraph 4/4 回归 ✅；语法检查 11 个文件全过。

**v0.42** Squad 三项产品化增强（role badge / 用户中途插话 / QA 严格度档位）✅：(1) **role badge**——`renderRoomMembers` chip 加 PM/DEV/QA/OBSERVER 圆角彩色徽章，紫/橙/绿/灰四色分边，配合 v0.40 已实现的 model 下拉，squad 房成员组成一目了然。(2) **用户中途插话**——`task.userInjections[{at,content}]` 字段；新 endpoint `POST /api/rooms/:id/tasks/:tid/inject` 把指示写入 task；`DEV_PROMPT` 在反馈段前新增"⚠️ 用户中途追加的指示（必须优先满足）"段，下次 Dev 重做时看见；详情抽屉里非 done 状态都出"📨 给本 task 追加指示"输入框 + 注入按钮，注入后 WS 广播 `task_injection_added` 并自动重渲染。专治 QA 死循环或 PM 拆错方向需要用户介入的场景。(3) **QA 严格度档位**——`room.qaStrictness = 'loose'|'standard'|'strict'`，三档对应 QA_PROMPT 中三段判定标准（loose：核心功能能用即可；standard：验收标准全达成；strict：100% + 可读性 + 错误处理 + 边界覆盖 + 风格统一，没问题也要找 1-2 个改进点）。PATCH `/api/rooms/:id` 接 `qaStrictness` 字段；前端房间头部 `room-debate-actions` 加下拉，仅 squad 房显示，change 即 PATCH。**冒烟**：默认 standard ✅、PATCH 切 strict 落库 ✅、注入端点 404 校验正确 ✅。

**v0.41** Squad 协作工作流（PM/Dev/QA 角色 + 任务依赖图 + Dev-QA 返工循环）✅：房间新增第二种模式 `mode='squad'`（保留 v0.39 debate）。新数据：`room.taskList` + `member.role`（pm/dev/qa/observer）+ `task.dependencies/iterations/maxIterations/attempts/reviews`。新代码：**`TaskGraph`**（DAG 拓扑排序 + 环检测 + readyTasks + allDoneOrTerminal，5/5 单测通过）；**`CollaborationDispatcher`** 编排：**Step1** PM 角色（默认 Claude）按用户 topic 输出严格 JSON taskList（含 assignee/reviewer/dependencies），失败容错 + 环检测则退化线性 → **Step2** TaskGraph.readyTasks 找无依赖 task → **Step3** 并行 Dev（每 task 的 assignee 实现）→ **Step4** Dev 完成自动转 QA → **Step5** QA 输出严格 JSON verdict（pass/reject + issues + suggestions + reasoning），reject 自动打回 Dev 附 issues 重做（iterations++）→ **Step6** iterations≥maxIterations(5) 强制 escalate 等用户介入 → **Step7** 全部 done 后 PM 合成最终交付。WS 新事件：`squad_start / pm_planning / plan_done / plan_cycle_fixed / batch_start / task_dev_start / task_dev_done / task_qa_start / task_qa_done / task_done / task_escalated / final_summary_start / final_summary_done / squad_done / squad_paused / squad_error`。前端：sidebar 💬 改成两个建房按钮（`＋ 新 Debate 房` / `＋ 新 Squad 房`）；Squad 房展示 **5 列 Kanban**（待开始/Dev 中/QA 中/已通过/升级）+ 任务卡片含迭代次数 + 打回次数 + 依赖链；点卡片弹**右侧详情抽屉**显示所有 attempts（🔨）+ reviews（✅/❌）按时间排序的完整 history（含每条 issues/suggestions/reasoning）。Squad 默认成员 1 PM(Claude) + 1 Dev(Claude) + 1 Dev(Codex) + 1 QA(Codex)，跨模型审查保证客观。**真测**：轻量 topic（写 3 行 Python hello world 带方括号）→ PM 拆 3 task 带 T1→T2→T3 依赖链 → 正常驱动执行。

**v0.40** 房间成员可选模型 + Watcher 多 provider 池（per-session 选谁监视）✅：(1) 房间侧——`room.members[].model` 字段新增，3 个 `RoomAdapter.chat` 都收 `opts.model` 覆盖（Claude `--model` / Codex `-m` / Ollama body.model），`DebateDispatcher` 在 R1-R3 + R4 judge 都把 member.model 透传。前端房间界面顶部加成员行（chip 含名字 + 模型下拉 + ✕/✓ 启用按钮），下拉硬编码常用模型清单（Claude: sonnet/opus/haiku; Codex: gpt-5/gpt-5-codex/o3/o3-mini; Ollama: gemma3:4b/qwen2.5:7b/llama3.2:3b/gpt-oss:20b），未在清单内的自定义值会自动追加为选项。PATCH `/api/rooms/:id` 接 `{ members }` 部分更新。(2) Watcher 侧——重构成 multi-provider 池：`watcherAdapterPool` Map 注册 4 个 provider（Claude/Codex/Ollama 永远在池，MiniMax 仅当 key+plan 支持时入池），新增 `ClaudeWatcherAdapter` / `CodexWatcherAdapter` 复用 `ClaudeSpawnAdapter` / `CodexSpawnAdapter` 拿文本 → `WatcherAdapter.validateVerdict` 解析 JSON。`session.watcherProviderId` 字段持久化（saveData/loadData/PATCH 全补），`WatcherDispatcher._pickAdapterFor(session)` 按 session 选 adapter 回退默认；`_performJudge(session, state, adapter)` 接 per-call adapter，broadcast 的 provider 字段也跟着 session 走。新端点 `GET /api/watcher/providers` 返回池里所有 provider 列表 + 默认 id。前端 chat-header 👁️ 监视按钮旁加 `<select id="watcherProviderSelect">` 下拉（仅启用时显示），切换即 PATCH session.watcherProviderId。**意义**：现在每个 session 可独立选"由谁监视"——Claude 用 GPT 监视、GPT 用 Claude 监视、敏感任务用本地 Ollama 监视都行；多个 session 不再共用同一 watcher。

**v0.39** 多 AI 聊天室（multi-agent debate）✅：用户下达一个任务 → 三个 AI 协同 4 轮讨论 → 输出一致最优方案。架构：抽象 `RoomAdapter` + `ClaudeSpawnAdapter`（spawn `claude --print` 走 20x plan 零增量）+ `CodexSpawnAdapter`（spawn `codex exec` 走 ChatGPT 5x plan 零增量）+ `OllamaChatAdapter`（gemma3:4b 本地零成本，顶位 MiniMax 直到用户 key 升级支持 chat）。`ChatRoomStore` 持久化 `~/.claude-panel/rooms.json`（房间 + members + rounds + finalConsensus）。`DebateDispatcher` 4 轮编排：**R1 独立提案**（并行调三方互不可见）→ **R2 互评修订**（看 R1 全部 → 评价 + 修订）→ **R3 终稿表态**（标共识点+分歧）→ **R4 Claude judge**（合成共识方案 + 分歧裁决 + 实施清单）。WS `/ws/room/:id` 实时推送 `debate_start / round_start / turn_start / turn_done / turn_error / round_done / judge_start / judge_done / debate_done / debate_paused / debate_error`。HTTP `/api/rooms` CRUD + `/debate` 启动 + `/abort` 中断。前端 sidebar 加 💬 按钮 → 独立 `room-area`（240px 房间列表 + 主区任务输入 + 4 轮卡片网格 + 共识方案突出色块）；状态 chip 5 态（idle/running/done/paused/error）+ speaking 动画 + 中文 markdown 渲染。**真测**：轻量 topic（早餐 10 字方案）完整跑通 ~90s（R1-R3 并行 60s + R4 judge 30s），三方在 R2 自然收敛到"燕麦/蓝莓/蛋奶/坚果"，R4 输出 2:1 票数裁决 + 7 步实施清单，0 token 增量。

**v0.38** Watcher 真用反馈修复轮（cycle_27-30）✅：5 场景真测发现 P0×2 + P1×2 + P2×2，全部修复 + 单测覆盖。**(1) P0-A status 解析**——gemma3:4b 偶发把枚举当值输出（`"completed | partial | stuck"`），validateVerdict 加 `split(/[|,]/) + trim quote` 容错，next_action.type 同样处理，5+1 用例通过；**(2) P0-B 中断误触 watcher**——SIGINT 后 exit code=0 让 watcher 误判 turn 完成，加 `session._lastInterrupted` 一次性标记，interrupt + reset-busy 端点都置 true，exit handler 检测到则跳过 watcher 触发并发 `turn_end` 事件（避免双 busy=false 广播）；**(3) P1 confidence 偏置**——buildJudgePrompt 重写：字段定义/示例分离 + 0.0-1.0 五段评分标准 + "不要恒填 0.95" 显式约束 + reasoning 强制中文，再补 4 类一致性 sanity（completed+remaining>0 / completed+items=0 / partial+全空 → 降 confidence；reasoning >60% ASCII 视为偷懒标记）；**(4) P2 cost 算成 default**——`findPricingKey` 三层（精确 → 前缀 `startsWith` → 关键词 opus/sonnet/haiku）解决带版本后缀（`claude-opus-4-7-20251225`）落 default 的低估，7/7 用例通过；**(5) P2 双广播**——exit handler wasInterrupted 时不再重发 busy=false 仅发 exit info。**冒烟测**：node --check 全过；PORT=7788 启动 → version/sessions/watcher/config 三端点正常返回 → graceful SIGTERM 关停。

**v0.37** Watcher Phase 1.6 — 历史 review + Settings UI（**Watcher Phase 1 完整结束**）✅：🛑 安全 tab 底部加 👁️ 监视者段：**配置盒**（启用/自动模式 toggle + provider 选择 + model/apiKey/baseUrl 输入 + 测试连通 / 保存按钮）+ **历史时间线**（每条 verdict 含 status pill + confidence + provider chip + 时间戳 + reasoning 200 字 + next_action 120 字 prompt，按 status 5 色板分边条）。前端按钮：测试连通直接调 `/api/watcher/test`，保存调 PUT `/api/watcher/config`（脱敏 apiKey 不覆盖原值）。
