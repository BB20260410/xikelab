...
const PM_PROMPT = (topic, members) => `# 你的角色：squad PM（项目经理）
...
### 队员名单
${members.map(m => `- ${m.displayName}（role=${m.role}${m.specialty ? '/' + m.specialty : ''}）adapterId=${m.adapterId}`).join('\n')}
...
const DEV_PROMPT = (task, topic, previousReviews) => {
...
const QA_PROMPT = (task, devContent, topic, strictness) => `# 你的角色：squad QA（审查员）
...
export class CollaborationDispatcher {
    ...
    try {
      ...
      if (isResume && Array.isArray(room.taskList) && room.taskList.length > 0) {
        ...
        const planRaw = await this._callAdapter(pms[0], [
          ...
          { role: 'user', content: PM_PROMPT(topic, members) },
    ...
    while (task.iterations < task.maxIterations) {
      ...
      try {
        devContent = await this._callAdapter(dev, [
          ...
          { role: 'user', content: DEV_PROMPT(task, topic, task.reviews) },
        ], room.cwd, abortSignal, { room, turnKind: `dev:${task.id}#i${task.iterations}` });
      } catch (e) {
      ...
      try {
        ...
        const qaRaw = await this._callAdapter(qa, [
          ...
          { role: 'user', content: QA_PROMPT(task, devContent, topic, strictness) },
        ], room.cwd, abortSignal, { room, turnKind: `qa:${task.id}#i${task.iterations}` });
  ...
  async _callAdapter(member, messages, cwd, abortSignal, ctx = {}) {
    ...
    try {
      result = await adapter.chat(finalMessages, { cwd, abortSignal, model: member.model });
    } catch (e) {
      err = e;
    }
